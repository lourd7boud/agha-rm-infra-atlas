/**
 * ═══════════════════════════════════════════════════════════════════════════════════
 * 📊 IMPORT MOROCCO OFFICIAL INDEXES - Direct SQL Execution
 * ═══════════════════════════════════════════════════════════════════════════════════
 * 
 * Usage: node import-morocco-indexes-direct.js
 * 
 * This script inserts ALL official Moroccan indexes for year 2024
 * Including: Index Simples + Index Globaux + Salaires + Charges Sociales
 */
...
/**
 * ═══════════════════════════════════════════════════════════════════════════════════
 * 📋 LISTE COMPLÈTE DES INDEX OFFICIELS MAROCAINS
 * ═══════════════════════════════════════════════════════════════════════════════════
 * 
...
// Full data for 2024 - ALL official indexes (59 indexes)
const FULL_DATA_2024 = [
  {
    month: '2024-01-01',
...
async function main() {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║   📊 MOROCCO OFFICIAL INDEX IMPORT                                    ║');
  ...
  try {
    for (const record of FULL_DATA_2024) {
      try {
        ...
        if (existing.rows.length > 0) {
          // Update existing - REPLACE all indexes
          await pool.query(`
            UPDATE revision_indexes 
            SET index_values = $1::jsonb,
                source = $2,
                notes = $3,
                updated_at = CURRENT_TIMESTAMP
            WHERE month_date = $4
          `, [JSON.stringify(record.indexes), record.source, record.notes, record.month]);
          
          updated++;
          console.log(`🔄 Updated: ${record.month} (${Object.keys(record.indexes).length} indexes)`);
        } else {
          // Insert new
          await pool.query(`
            INSERT INTO revision_indexes (month_date, index_values, source, notes)
            VALUES ($1, $2, $3, $4)
          `, [record.month, JSON.stringify(record.indexes), record.source, record.notes]);
          
          imported++;
          console.log(`✅ Imported: ${record.month} (${Object.keys(record.indexes).length} indexes)`);
        }
      } catch (err) {
        errors++;
        console.error(`❌ Error for ${record.month}:`, err.message);
      }
    }
    ...
    console.log(`  📅 Total months: ${FULL_DATA_2024.length}`);
    console.log(`  📊 Indexes per month: ${Object.keys(FULL_DATA_2024[0].indexes).length}`);
    console.log('═'.repeat(70));
    ...
    const result = await pool.query(`
      SELECT 
        ...
        (SELECT jsonb_object_keys(index_values) FROM revision_indexes LIMIT 1) as sample_index
      FROM revision_indexes
    `);
    console.log(`  📅 Total months in DB: ${result.rows[0].total_months}`);
    console.log(`  📅 First month: ${result.rows[0].first_month}`);
    console.log(`  📅 Last month: ${result.rows[0].last_month}`);
    
    // Count unique indexes
    const indexCount = await pool.query(`
      SELECT COUNT(DISTINCT key) as index_count
      FROM revision_indexes, jsonb_each(index_values)
    `);
    console.log(`  📊 Unique indexes: ${indexCount.rows[0].index_count}`);
    
    // List all indexes
    const indexList = await pool.query(`
      SELECT DISTINCT key as index_name
      FROM revision_indexes, jsonb_each(index_values)
      ORDER BY key
    `);
    console.log('');
    ...
    console.log('─'.repeat(70));
    const indexNames = indexList.rows.map(r => r.index_name);
    for (let i = 0; i < indexNames.length; i += 10) {
      console.log('  ' + indexNames.slice(i, i + 10).join(', '));
    }
