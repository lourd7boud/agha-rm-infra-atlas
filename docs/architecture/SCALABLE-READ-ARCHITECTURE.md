...
- **IN PROGRESS: P2** — write-time classification columns (region/ville/category/secteur/lot_count/has_bpu) + backfill + DB-side `WHERE`/`ORDER BY`/`LIMIT`/`OFFSET` + `GROUP BY` facets, so the SERVER also becomes O(page) (today `selectInventory` still classifies all rows in JS per request — fast at 5989, O(n) at 100k). Then P4 (roll the contract to remaining surfaces: orchestrator/projects/finance/…), P5 (infra). Optional: `WATCH_CONTINUOUS=false` (periodic crawl), drop `aiResume` from the list.
