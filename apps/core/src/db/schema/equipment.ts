...
export const equipmentWorkOrders = equipment.table(
  'work_order',
  {
    ...
    equipmentId: uuid('equipment_id')
      .notNull()
      .references(() => equipments.id),
    // When set, this work order was generated by a preventive maintenance plan;
    // closing it advances that plan's last-service baseline. Null for corrective.
    // ON DELETE SET NULL: deleting a plan detaches (keeps) its work-order history.
    planId: uuid('plan_id').references(() => equipmentMaintenancePlans.id, {
      onDelete: 'set null',
    }),
    type: text('type').notNull(),
  ...
  (table) => [
    ...
    // The plan idempotency check filters by plan_id — keep it off a seq scan.
    index('equipment_work_order_plan_id_idx').on(table.planId),
    check(
...
// ── Maintenance plans — plans d'entretien préventif ──────────────────────────
// A recurring service defined per machine, firing off usage ('meter': every
// interval_meter heures/km past last_service_meter) or the calendar ('temps':
// every interval_days past last_service_date). When due it generates a
// preventive work_order (linked via work_order.plan_id); closing that WO
// advances the plan's last-service baseline. trigger_type mirrors
// MAINTENANCE_TRIGGER_TYPES in equipment.maintenance.domain.
export const equipmentMaintenancePlans = equipment.table(
  'maintenance_plan',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    equipmentId: uuid('equipment_id')
    ...
    // meter plans: unit + interval + baseline (last serviced at this meter).
    meterUnit: text('meter_unit'),
    intervalMeter: numeric('interval_meter', { precision: 12, scale: 2 }),
    lastServiceMeter: numeric('last_service_meter', { precision: 12, scale: 2 }),
    // time plans: interval in days + baseline (last serviced on this date).
    intervalDays: integer('interval_days'),
    lastServiceDate: date('last_service_date', { mode: 'date' }),
    active: boolean('active').notNull().default(true),
    notes: text('notes'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    index('equipment_maintenance_plan_equipment_id_idx').on(table.equipmentId),
    check(
      'equipment_maintenance_plan_trigger_check',
      sql`${table.triggerType} IN ('meter', 'temps')`,
    ),
    // Defence-in-depth like the other enum columns: meter_unit is nullable
    // (time plans have none) but, when set, must be a recognised unit.
    check(
